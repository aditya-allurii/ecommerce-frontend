...
export const authenticateSignInUser 
    = (sendData, toast, reset, navigate, setLoader, isAdminLogin = false) => async (dispatch) => {
        try {
            setLoader(true);
            const { data } = await api.post("/auth/signin", sendData);
            const roles = data.roles || [];
            if (isAdminLogin && !roles.includes("ROLE_ADMIN") && !roles.includes("ROLE_SELLER")) {
            ...
            reset();
            toast.success(isAdminLogin ? "Admin Login Success" : "Login Success");
...
export const createStripePaymentSecret 
    = (sendData) => async (dispatch, getState) => {
        try {
            ...
            console.log(error);
            const msg = error?.response?.data?.message || "Failed to create client secret";
            toast.error(msg);
...
export const placeCodOrder 
    = (sendData, setErrorMesssage, setLoading, navigate, toast) => async (dispatch, getState) => {
        try {
            setLoading(true);
            const response  = await api.post("/order/users/payments/cash-on-delivery", sendData);
            if (response.data) {
                localStorage.removeItem("CHECKOUT_ADDRESS");
                localStorage.removeItem("cartItems");
                localStorage.removeItem("client-secret");
                ...
                navigate("/order-confirm?payment_method=cash-on-delivery");
              } else {
                setErrorMesssage("Failed to place order. Please try again.");
              }
        } catch (error) {
            console.log(error);
            setErrorMesssage(error?.response?.data?.message || "Failed to place order. Please try again.");
        } finally {
            setLoading(false);
        }
};
...
export const getOrdersForDashboard = (queryString, isAdmin, isProfileOrders = false) => async (dispatch) => {
    try {
        dispatch({ type: "IS_FETCHING" });
        const endpoint = isProfileOrders 
            ? "/users/orders" 
