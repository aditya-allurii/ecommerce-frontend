        ...
        <div className="grid md:grid-cols-5 grid-cols-4 md:text-md text-sm gap-4   items-center  border border-slate-200  rounded-md  lg:px-4  py-4 p-2">
            <div className="md:col-span-2 justify-self-start flex  flex-col gap-2 ">
                ...
                <div className="md:w-36 sm:w-24 w-12">
                    <img 
                        src={image && (image.startsWith('http') || image.startsWith('/products') || image.startsWith('products')) ? (image.startsWith('/') ? image : '/' + image) : `${import.meta.env.VITE_BACK_END_URL}/images/${image}`}
                        alt={productName}
                        className="md:h-36 sm:h-24 h-12 w-full object-contain rounded-md bg-white"/>
