            ...
            <div className='space-y-2'>
                {cart?.map((item) => (
                    <div key={item?.productId} className='flex items-center'>
                        <img src={item?.image && (item.image.startsWith('http') || item.image.startsWith('/products') || item.image.startsWith('products')) ? (item.image.startsWith('/') ? item.image : '/' + item.image) : `${import.meta.env.VITE_BACK_END_URL}/images/${item?.image}`}
                        alt='Product'
                        className='w-12 h-12 object-contain rounded-sm bg-white'></img>
